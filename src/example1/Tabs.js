...
const Selector = (classPrefix) => ({
  PREFIX: classPrefix,
  NAV: `${classPrefix}-nav`, CONTENT: `${classPrefix}-content`, TAB: `${classPrefix}-tab`,
...
class Tabs {
  ...
  _switchTabs(toIndex) {
    ...
    tabs
      .eq(fromIndex).removeClass(this.selector.ACTIVE).attr('aria-selected', false);
    tabs
      .eq(toIndex).addClass(this.selector.ACTIVE).attr('aria-selected', true);
  }
  ...
  _getPanelInfo(toIndex) {
    ...
    return {
      toIndex: toIndex, fromIndex: fromIndex, toPanels: $(toPanels), fromPanels: $(fromPanels),
    };
  }
